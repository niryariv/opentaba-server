...
def get_plans(gush_id):
    ...
    plans = db.plans.find({"gush_id": gush_id}).sort([("year", pymongo.DESCENDING),
...
@app.route('/<gush_id>/feed')
def atom_city_feed(gush_id):
    feed = AtomFeed("OpenTABA_gush_%s" % gush_id,
    ...
    for p in plans_clean:
        url = 'http://mmi.gov.il/IturTabot/taba4.asp?' + url_encode({'kod': 3000,
                                                                     'MsTochnit': p['number']},
...
@app.route('/')
def hello():
    out = '''<html><body style="font-size: 3em; margin: 100px;
    text-align:center">
    <p>Hi. You\'ve reached the server side of
    <a href="http://opentaba.info">opentaba.info</a></p>
